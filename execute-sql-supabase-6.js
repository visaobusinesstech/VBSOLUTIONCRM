// Script para executar SQL no Supabase - Tentativa 6
const https = require('https');

console.log('🎯 Executando SQL no Supabase - Tentativa 6...');
...
async function executeSQLSupabase6() {
    try {
        console.log('📡 Conectando ao Supabase...');
        
        // 1. Tentar usar a função de execução de SQL personalizada
        console.log('📝 1. Tentando usar função de execução de SQL personalizada...');
        const execUrl = `${SUPABASE_URL}/rest/v1/rpc/exec_sql_custom`;
        const execOptions = {
            method: 'POST',
            headers: {
        ...
        const execResult = await makeRequest(execUrl, execOptions);
        
        if (execResult.status === 200) {
            console.log('✅ Tabela pipelines criada via exec_sql_custom');
            ...
            console.log('❌ Erro ao executar exec_sql_custom:', execResult.data);
            return false;
        }
...
executeSQLSupabase6().then(success => {
    if (success) {
        console.log('\n✅ Configuração concluída com sucesso!');
        console.log('🚀 Agora você pode testar o gerenciamento de pipelines');
    } else {
        console.log('\n❌ Tentativa 6 falhou, tentando método alternativo...');
    }
});
