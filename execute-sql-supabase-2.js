// Script para executar SQL no Supabase - Tentativa 2
const https = require('https');

console.log('🎯 Executando SQL no Supabase - Tentativa 2...');
...
async function executeSQLSupabase2() {
    try {
        console.log('📡 Conectando ao Supabase...');
        
        // 1. Tentar usar a função de administração
        console.log('📝 1. Tentando usar função de administração...');
        ...
        if (adminResult.status === 200) {
            console.log('✅ Tabela pipelines criada via administração');
            ...
            console.log('❌ Erro ao executar administração:', adminResult.data);
            return false;
        }
...
executeSQLSupabase2().then(success => {
    if (success) {
        console.log('\n✅ Configuração concluída com sucesso!');
        console.log('🚀 Agora você pode testar o gerenciamento de pipelines');
    } else {
        console.log('\n❌ Tentativa 2 falhou, tentando método alternativo...');
    }
});
