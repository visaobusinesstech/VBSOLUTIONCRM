...
export default function GoogleIntegrationModal({
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/integrations/google/auth', {
        method: 'GET',
        headers: {
        ...
        const checkClosed = setInterval(() => {
          if (popup?.closed) {
            ...
            const checkConnection = async () => {
              try {
                const statusResponse = await fetch('http://localhost:3000/api/integrations/google/status');
                const statusData = await statusResponse.json();
                
                if (statusData.success && statusData.data.connected) {
                  setStep('success');
                  setTimeout(() => {
                    onSuccess?.({ platform: 'google', connected: true, userEmail: statusData.data.userEmail });
                    onClose();
                  }, 2000);
                } else {
                  setStep('error');
