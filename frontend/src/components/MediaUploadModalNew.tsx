...
import { Upload, X, Image as ImageIcon, Video, Calendar, Clock, MapPin } from 'lucide-react';
import { toast } from 'sonner';
import { CreateFeedPostData } from '@/types/feed';

interface MediaUploadModalNewProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (postData: CreateFeedPostData) => void;
  type: 'image' | 'video' | 'event';
}

const MediaUploadModalNew = ({ isOpen, onClose, onSubmit, type }: MediaUploadModalNewProps) => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        toast.error(`O arquivo deve ter no máximo ${type === 'video' ? '50MB' : '10MB'}`);
        return;
      }
  ...
  const handleSubmit = () => {
    if (type === 'event') {
      if (!eventData.title || !eventData.date || !eventData.time) {
        toast.error('Por favor, preencha título, data e horário do evento');
        return;
      }
      ...
      onSubmit({
        type: 'event',
        content: eventData.location || content,
        event_date: eventDateTime.toISOString()
      });
    } else {
      if (!content.trim() && !mediaFile) {
        toast.error('Por favor, adicione conteúdo ou selecione um arquivo');
        return;
      }
  ...
  const getPlaceholder = () => {
    switch (type) {
      case 'image': return 'Descreva sua imagem...';
      case 'video': return 'Descreva seu vídeo...';
      case 'event': return 'Adicione detalhes sobre o evento...';
      default: return 'O que você gostaria de compartilhar?';
    }
  };
...
export default MediaUploadModalNew;
