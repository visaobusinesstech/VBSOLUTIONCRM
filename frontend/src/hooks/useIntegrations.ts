...
export function useIntegrations() {
  ...
  const fetchIntegrations = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/integrations/integrations', {
        headers: {
          'x-user-id': user.id,
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/integrations/integrations/status', {
        headers: {
          'x-user-id': user.id,
          'Content-Type': 'application/json',
        },
