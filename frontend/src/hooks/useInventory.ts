...
export const useInventory = () => {
  ...
  const fetchInventoryItems = async () => {
    ...
    try {
      ...
      // Buscar itens do inventário - tentar diferentes abordagens
      ...
      // Se der erro 403 ou 400, tentar sem filtro de owner_id (deixar RLS resolver)
      if (error && (error.code === 'PGRST301' || error.message.includes('403') || error.message.includes('400'))) {
        console.log('🔄 fetchInventoryItems: Tentando sem filtro owner_id...');
        ...
        if (errorNoFilter) {
          console.error('❌ fetchInventoryItems: Erro na consulta sem filtro:', errorNoFilter);
          throw errorNoFilter;
        }
  ...
  const createInventoryItem = async (itemData: {
    ...
    try {
      ...
      console.log('📦 createInventoryItem: Dados do produto:', productData);
      ...
      console.log('📋 createInventoryItem: Dados do inventário:', inventoryData);
      ...
      if (inventoryError) {
        ...
        console.error('📋 Detalhes do erro:', JSON.stringify(inventoryError, null, 2));
  ...
  const updateInventoryItem = async (id: string, updates: {
    ...
    try {
      ...
      if (updates.name || updates.category || updates.price || updates.min_stock || updates.description || updates.image_url) {
        ...
        if (productError) {
          console.error('❌ updateInventoryItem: Erro ao atualizar produto:', productError);
          throw productError;
        }
      }
      ...
      if (updates.quantity !== undefined) {
        ...
        if (error) {
          console.error('❌ updateInventoryItem: Erro ao atualizar inventário:', error);
          throw error;
        }
      ...
      console.error('❌ updateInventoryItem: Erro ao atualizar item do inventário:', err);
      throw err;
    }
  };
  ...
  const deleteInventoryItem = async (id: string) => {
    ...
    try {
      ...
      if (inventoryError) {
        console.error('❌ deleteInventoryItem: Erro ao deletar inventário:', inventoryError);
        throw inventoryError;
      }
      ...
      if (productError) {
        console.error('❌ deleteInventoryItem: Erro ao deletar produto:', productError);
        throw productError;
      }
      ...
      console.error('❌ deleteInventoryItem: Erro ao deletar item do inventário:', err);
      throw err;
    }
  };
