...
export function useCalendar() {
  ...
  // Usar email como ID temporário ou um ID fixo para testes
  const userId = userEmail || 'test-user-id';
  const { integrations, isPlatformConnected } = useIntegrations();
  const googleCalendar = useGoogleCalendar(userId);
  ...
  }, [userId, fetchLocalEvents, fetchGoogleEvents]);
  ...
  }, [userId, createLocalEvent, googleCalendar, getAllEvents]);
  ...
  const refreshEvents = useCallback(async () => {
    ...
    setEventsLoaded(false); // Reset para permitir novo carregamento
  }, []);
    ...
    if (userId && !eventsLoaded) {
      ...
      getAllEvents()
        ...
        .catch((error) => {
          ...
          setEventsLoaded(true); // Marcar como carregado mesmo com erro para evitar loop
        });
    }
  }, [userId, googleCalendar.isConnected, eventsLoaded]); // Adicionado eventsLoaded
