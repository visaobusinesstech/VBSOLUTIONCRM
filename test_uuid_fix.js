...
async function testUuidFix() {
    try {
        console.log('🎯 Teste UUID Fix - Verificando problemas de UUID...');
        ...
        // 2. Testar criação de lead com stage_id válido
        console.log('🧪 2. Testando criação de lead com stage_id válido...');
        const validLead = {
            name: 'Lead UUID Teste - ' + new Date().toISOString(),
            email: 'teste@exemplo.com',
            phone: '11999999999',
            company: 'Empresa UUID Teste',
            source: 'website',
            priority: 'medium',
        ...
        // 3. Testar criação de lead com stage_id inválido (simulando o problema)
        console.log('🧪 3. Testando criação de lead com stage_id inválido (simulando problema)...');
        const invalidLead = {
            name: 'Lead UUID Inválido - ' + new Date().toISOString(),
            email: 'teste@exemplo.com',
            phone: '11999999999',
            company: 'Empresa UUID Inválido',
            source: 'website',
            priority: 'medium',
            stage_id: '1', // UUID inválido (simulando o problema)
            status: 'cold',
            currency: 'BRL',
            value: 1000
        };

        console.log('📋 Dados com stage_id inválido:', invalidLead);

        const invalidResult = await makeRequest(createUrl, createOptions);
        
        console.log('📋 Status da criação inválida:', invalidResult.status);
        console.log('📋 Dados da resposta:', invalidResult.data);

        if (invalidResult.status === 201) {
            console.log('⚠️ Lead com stage_id inválido foi criado (não deveria acontecer)');
            
            // Deletar o lead
            const deleteUrl = `${SUPABASE_URL}/rest/v1/leads?id=eq.${invalidResult.data[0].id}`;
            const deleteOptions = {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
            ...
            console.log('✅ Lead com stage_id inválido foi rejeitado (comportamento esperado)');
            console.log('📋 Erro esperado:', invalidResult.data);
        }

        console.log('🎉 Teste UUID Fix concluído!');
        console.log('📋 Resumo:');
        console.log('   - API Supabase está funcionando');
        console.log('   - Criação de leads com stage_id válido está funcionando');
        console.log('   - Criação de leads com stage_id inválido é rejeitada');
        console.log('   - Frontend deve validar stage_id antes de enviar');

    } catch (error) {
        console.error('❌ Erro no teste UUID fix:', error);
    }
}
