// Script para executar SQL no Supabase - Tentativa 5
const https = require('https');

console.log('🎯 Executando SQL no Supabase - Tentativa 5...');
...
async function executeSQLSupabase5() {
    try {
        console.log('📡 Conectando ao Supabase...');
        
        // 1. Tentar usar a função de migração personalizada
        console.log('📝 1. Tentando usar função de migração personalizada...');
        const migrationUrl = `${SUPABASE_URL}/rest/v1/rpc/migrate_schema`;
        ...
        if (migrationResult.status === 200) {
            console.log('✅ Tabela pipelines criada via migrate_schema');
            ...
            console.log('❌ Erro ao executar migrate_schema:', migrationResult.data);
            return false;
        }
...
executeSQLSupabase5().then(success => {
    if (success) {
        console.log('\n✅ Configuração concluída com sucesso!');
        console.log('🚀 Agora você pode testar o gerenciamento de pipelines');
    } else {
        console.log('\n❌ Tentativa 5 falhou, tentando método alternativo...');
    }
});
