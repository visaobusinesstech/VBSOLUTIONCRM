// Script para executar SQL no Supabase - Tentativa 1
const https = require('https');

console.log('🎯 Executando SQL no Supabase - Tentativa 1...');
...
async function executeSQLSupabase1() {
    try {
        console.log('📡 Conectando ao Supabase...');
        
        // 1. Tentar usar a função de migração
        console.log('📝 1. Tentando usar função de migração...');
        ...
        if (migrationResult.status === 200) {
            console.log('✅ Tabela pipelines criada via migração');
...
executeSQLSupabase1().then(success => {
    if (success) {
        console.log('\n✅ Configuração concluída com sucesso!');
        console.log('🚀 Agora você pode testar o gerenciamento de pipelines');
    } else {
        console.log('\n❌ Tentativa 1 falhou, tentando método alternativo...');
    }
});
