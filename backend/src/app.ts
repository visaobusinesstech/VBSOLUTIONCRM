...
export function createApp(io: any) {
  ...
  // Rota temporária para testar Google OAuth
  app.get('/api/integrations/google/auth', (req, res) => {
    try {
      ...
      const oauth2Client = new google.auth.OAuth2(
        INTEGRATIONS_CONFIG.GOOGLE.CLIENT_ID,
        INTEGRATIONS_CONFIG.GOOGLE.CLIENT_SECRET,
        INTEGRATIONS_CONFIG.GOOGLE.REDIRECT_URI
      );

      const authUrl = oauth2Client.generateAuthUrl({
        access_type: 'offline',
        scope: INTEGRATIONS_CONFIG.GOOGLE.SCOPES,
        prompt: 'consent'
      });
